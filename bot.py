#!/usr/bin/env python3
# bot_python_only_v4_render_fixed.py
# Full Flask + Webhook for Render.com - FIXED VERSION
...
# ✅ FIX: Render.com এর জন্য সঠিক webhook URL
...
def forward_upload_to_admins(uploader_msg, saved_path: str):
    ...
    info_text = (
        ...
        f"👤 From: {sender.first_name or ''} {getattr(sender, 'last_name', '') or ''}\n"
        f"🔗 Username: @{sender.username if getattr(sender, 'username', None) else 'N/A'}\n"
        f"🆔 UserID: {sender.id}\n"
...
def send_log_or_file(chat_id: int, text: str, filename_hint: str = "log.txt"):
    ...
    try:
        if not text:
            bot.send_message(chat_id, "📜 No log available.")
            return
        if len(text) <= TELEGRAM_MAX_MSG:
            bot.send_message(chat_id, f"📜 Log tail:\n\n{text}")
            return
        # try sending as file
        ...
        try:
            with open(tmp_path, "rb") as doc:
                bot.send_document(chat_id, doc, caption="📜 Full log attached")
        except Exception as e:
            logger.warning("Sending log as document failed: %s", e)
            # fallback to chunked messages
            send_long_message(chat_id, text, prefix="📜 Log is large; sending in parts:")
        finally:
            try:
                os.remove(tmp_path)
            except:
...
def speed(m):
    ...
    bot.edit_message_text(f"⚡ Response {elapsed} ms", msg.chat.id, msg.message_id)
...
def upload_hint(m):
    bot.reply_to(m, "📤 Send me a file: any type. .zip files containing .py are supported. Max size per file: {} MB".format(MAX_FILE_SIZE // (1024 * 1024)))

@bot.message_handler(func=lambda m: m.text == "🤖 Git Clone")
def git_clone_prompt(m):
    uid = m.from_user.id
    if uid in banned_users:
        bot.reply_to(m, "❌ You are banned.")
        return
    bot.reply_to(m, "🔗 Send the repo URL (https://github.com/owner/repo.git) or 'cancel' to abort.")
...
def check_files_ui(m):
    ...
    if not lst:
        bot.reply_to(m, "📂 You have no visible files. Upload .py or a .zip with a main file.")
        return
    markup = types.InlineKeyboardMarkup(row_width=1)
    for idx, (display, full, ftype) in enumerate(lst):
        status = "🟢" if f"{uid}_{os.path.basename(full)}" in running_scripts and running_scripts.get(f"{uid}_{os.path.basename(full)}", {}).get("proc") else "⏯️"
        markup.add(types.InlineKeyboardButton(f"{status} {display}", callback_data=f"file_{idx}"))
    bot.reply_to(m, "📂 Your Files (only main files are shown):", reply_markup=markup)
...
def generic_text(m):
    ...
    if state.get("expecting_clone_link"):
        if txt.lower() in ("cancel", "stop"):
            ...
            return
        repo = txt
        bot.reply_to(m, "⏳ Cloning repository... please wait (this can take a while).")
        success, result = clone_repo_and_process(uid, repo)
        if not success:
            bot.reply_to(m, f"❌ Clone failed: {result}")
            pending_prompts.pop(uid, None)
            return
        ...
        if mains:
            ...
            bot.reply_to(m, f"{'✅ Cloned and started' if ok else '⚠️ Cloned but failed to start'}: {msg}")
            pending_prompts.pop(uid, None)
            return
        else:
            # ask user to provide main filename
            pending_prompts[uid] = {"expecting_main_from_clone": clone_folder}
            bot.reply_to(m, "⚠️ No prioritized main file found automatically. Please reply with the filename to run (relative path within repo, e.g. app/main.py or main.py).")
            return
    ...
    if state.get("expecting_main_from_extract"):
        ...
        if not candidate:
            bot.reply_to(m, f"❌ File '{chosen}' not found in extracted folder.")
            return
        add_user_file(uid, os.path.relpath(candidate, get_user_folder(uid)), candidate, "main")
        ok, msg = start_python_script(uid, candidate)
        bot.reply_to(m, f"{'✅ Started' if ok else '❌ Failed to start'}: {msg}")
...
def clone_repo_and_process(user_id: int, repo_url: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if env_path:
            ...
            try:
                bot.send_message(user_id, f"⚠️ Found config file: {os.path.basename(env_path)} in the cloned repo.\nIf you want to edit or append values now (e.g. TOKEN=...), reply 'yes'. To skip reply 'no'.")
            except Exception:
                pass
        return True, target
    except Exception as e:
...
def handle_upload(m):
    ...
    if fsize and fsize > MAX_FILE_SIZE:
        bot.reply_to(m, f"❌ File too large ({fsize} bytes). Max is {MAX_FILE_SIZE}.")
        return

    processing = bot.reply_to(m, f"⬇️ Downloading {fname} ...")
    try:
        downloaded = bot.download_file(file_info.file_path)
    except Exception as e:
        bot.edit_message_text(f"❌ Download failed: {e}", processing.chat.id, processing.message_id)
        return
    ...
    if ext == ".zip":
        ...
        try:
            ...
            for p in py_files:
                if p in main_candidates[:len(main_candidates)] and os.path.basename(p).lower() in [n.lower() for n in MAIN_PRIORITY]:
                    ...
                    dep_count += 1
            msg = f"✅ ZIP extracted. {len(py_files)} .py files found, {dep_count} hidden as dependencies."
            # if no main auto-detected, ask user to provide main
            if not main_added:
                pending_prompts[uid] = {"expecting_main_from_extract": extract_folder}
                bot.edit_message_text(msg + "\n⚠️ No prioritized main found. Reply here with the filename to run (e.g. main.py).", processing.chat.id, processing.message_id)
                ...
                summary = msg + "\n"
                if started:
                    summary += f"🚀 Auto-started: {os.path.basename(started[0][0])} -> {started[0][2]}"
                bot.edit_message_text(summary, processing.chat.id, processing.message_id)
                if uid != OWNER_ID and uid not in ADMIN_IDS:
                    forward_upload_to_admins(m, save_path)
                return
        except zipfile.BadZipFile:
            bot.edit_message_text("❌ Invalid ZIP file.", processing.chat.id, processing.message_id)
            return
        except Exception as e:
            bot.edit_message_text(f"❌ Extraction error: {e}", processing.chat.id, processing.message_id)
            return
    else:
        ...
        if ext == ".py" and os.path.basename(save_path).lower() in [n.lower() for n in MAIN_PRIORITY]:
            ...
            bot.edit_message_text(f"✅ Uploaded and started: {res}" if ok else f"❌ Uploaded but failed: {res}", processing.chat.id, processing.message_id)
        else:
            ...
            bot.edit_message_text(f"✅ Uploaded: {os.path.basename(save_path)} (not auto-started). Use Check Files to manage.", processing.chat.id, processing.message_id)
...
def cb_file_panel(call):
    ...
    if is_running:
        markup.add(types.InlineKeyboardButton("🔴 Stop", callback_data=f"stop_{idx}"),
                   types.InlineKeyboardButton("📜 Logs", callback_data=f"logs_{idx}"))
    else:
        markup.add(types.InlineKeyboardButton("🟢 Start", callback_data=f"start_{idx}"),
                   types.InlineKeyboardButton("📜 Logs", callback_data=f"logs_{idx}"))
    markup.add(types.InlineKeyboardButton("🗑️ Delete", callback_data=f"delete_{idx}"),
               types.InlineKeyboardButton("◀ Back", callback_data="back_files"))
...
def cb_actions(call):
    ...
    if data == "back_files":
        ...
        try:
            bot.edit_message_text("📂 Your Files (main only):", call.message.chat.id, call.message.message_id, reply_markup=markup)
        except Exception as e:
...
def cmd_ban(m):
    ...
    save_users_state()
    bot.reply_to(m, f"✅ Banned {uid} and stopped running scripts.")
    try:
...
def auto_restore_running_scripts():
    ...
    for key, info in list(running_scripts.items()):
        ...
        if ok:
            try:
                bot.send_message(user_id, f"🔁 Your bot {basename} was automatically restarted after reboot.")
            except Exception:
                pass
        else:
...
# ✅ FIX: Correct webhook route using variable
...
if __name__ == "__main__":
    ...
    if not webhook_ok and not WEBHOOK_URL:
